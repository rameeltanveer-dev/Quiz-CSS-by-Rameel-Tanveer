/* script.js
   ...
   - Webhook (Google Apps Script) default URL
*/
...
const DEFAULT_WEBHOOK = 'https://script.google.com/macros/s/AKfycbx5mPGLMUXGsCcADkSD09ryOJcZEI9LHsJKMfwU_DCNNWrfdBUVUTyaw_CL1YVl_rpPS/exec';
...
function saveWebhook(url) {
    if(!url){
        ...
        webhookMsg.textContent = 'Webhook cleared.';
        return;
    }
    localStorage.setItem(WEBHOOK_KEY, url.trim());
    webhookMsg.textContent = 'Webhook saved.';
    webhookInput.value = url.trim();
}

function clearWebhook() {
    localStorage.removeItem(WEBHOOK_KEY);
    webhookInput.value = DEFAULT_WEBHOOK;
    webhookMsg.textContent = 'Webhook cleared. Using default.';
}

// ---------- REST OF QUIZ LOGIC ----------
// ... (Same as previous code: login, timers, loadQuestion, selectOption, finishQuiz, etc.)
